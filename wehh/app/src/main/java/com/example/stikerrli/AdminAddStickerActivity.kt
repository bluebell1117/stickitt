...
class AdminAddStickerActivity : AppCompatActivity() {
    ...
    private val imagePickerLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            result.data?.data?.let {
                ...
                binding.ivStickerPreview.setImageURI(uri)
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.ivBackAddSticker.setOnClickListener {
            finish()
        }
    }

    private fun saveSticker() {
        val name = binding.etStickerName.text.toString().trim()
        val priceStr = binding.etStickerPrice.text.toString().trim()
        val categoryIdStr = "1" // Placeholder, you should get this from the spinner
